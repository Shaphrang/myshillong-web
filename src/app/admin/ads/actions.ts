...
export async function createAd(formData: FormData) { await requireAdmin(); await sbMutate('sponsored_placements','POST', adPayload(formData)); revalidatePath('/admin/ads'); }
export async function updateAd(formData: FormData) { await requireAdmin(); const id=formData.get('id'); await sbMutate('sponsored_placements','PATCH', adPayload(formData), `id=eq.${id}`); revalidatePath('/admin/ads'); }
export async function deleteAd(formData: FormData) { await requireAdmin(); await sbMutate('sponsored_placements','DELETE',undefined,`id=eq.${formData.get('id')}`); revalidatePath('/admin/ads'); }
